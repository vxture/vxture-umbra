...
Backed up via timestamped file copies in `deploy/worker-03/scripts/55-backup-runtime-state.sh`.
...
Keys generated by [`13-generate-runtime-secrets.sh`](../../deploy/worker-03/scripts/13-generate-runtime-secrets.sh) and stored in `DATA_DIR/private/reality.json`:
...
| `deploy/worker-03/scripts/55-backup-runtime-state.sh` | Always creates new timestamped archive |
| `deploy/worker-03/scripts/53-manage-certificates.sh --upgrade` | Issues into `letsencrypt.staged`; production certs are untouched unless all domains verify trusted; partial staged successes are kept for retry |
| `deploy/worker-03/scripts/53-manage-certificates.sh --clean-workdirs` | Migrates legacy staged state and removes obsolete cert workdirs; active certs and backups are preserved |
| `deploy/worker-03/scripts/53-manage-certificates.sh --renew` | Delegates to certbot renew; does not force reissue |
