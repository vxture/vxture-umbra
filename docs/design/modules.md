...
- The dashboard reads the stored subscription URL on normal page views. The user `Update subscription URL` button and admin invite-console `Update URL` button call the same update path: refresh the current `subscription_url` from Marzban by bound user code, then update the local account cache.
